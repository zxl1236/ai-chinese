...
class SmartAIAssistant {
    ...
    async init() {
        ...
        // 🎯 立即检测当前页面的题目和内容
        setTimeout(() => {
            console.log('[AI] 执行初始检测...');
            
            // 🎯 确保写作区域可见，用户才能看到和使用输入框
            this.ensureWritingAreaVisible();
            
            this.updateStatus(); // 这会调用 updateTopic 和 updateContent
        }, 1000); // 增加延迟，确保页面动态内容完全加载
    }
    ...
    loadHTML() {
        ...
        // 直接使用内置HTML，不尝试加载外部文件
        console.log('📄 直接使用内置HTML模板');
        this.createHTML();
    }
    ...
    addCSS() {
        ...
        .ai-trigger{display:flex;align-items:center;gap:12px;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:#fff;padding:12px 16px;border-radius:50px;cursor:pointer;box-shadow:0 8px 25px rgba(102,126,234,.3);transition:all .3s cubic-bezier(.4,0,.2,1);-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none;position:relative;pointer-events:auto !important}
    ...
    // 🎯 确保写作区域可见（解决上方输入框不可用的问题）
    ensureWritingAreaVisible() {
        console.log('[AI] 确保写作区域可见...');
        ...
        const waitForElement = (selector, maxAttempts = 20) => {
            ...
            const check = () => {
                ...
                if (element || attempts >= maxAttempts) {
                    console.log(`[AI] ${selector} ${element ? '找到' : '超时未找到'} (尝试 ${attempts + 1} 次)`);
                    return element;
                }
                attempts++;
        ...
        console.log('[AI] 检查关键元素:');
        console.log('[AI] - customTitle:', customTitleInput ? '✅ 存在' : '❌ 不存在');
        console.log('[AI] - content-editor:', contentEditor ? '✅ 存在' : '❌ 不存在');
        ...
        if (writingArea && writingArea.style.display === 'none') {
            console.log('[AI] 显示写作区域');
            writingArea.style.display = 'block';
        }
        
        if (topicHeader && topicHeader.style.display === 'none') {
            console.log('[AI] 显示题目头部');
            topicHeader.style.display = 'block';
        }
        
        if (welcomePage && welcomePage.style.display !== 'none') {
            console.log('[AI] 隐藏欢迎页面');
            welcomePage.style.display = 'none';
        }
        
        // 设置默认题目到找到的输入框中
        const titleInput = customTitleInput || document.getElementById('current-title') || document.getElementById('article-title');
        if (titleInput && (!titleInput.value || titleInput.value === '请选择一个题目开始写作')) {
            console.log('[AI] 设置默认题目到:', titleInput.id || titleInput.className);
            titleInput.value = '我的理想';
            // 触发输入事件，确保监听器能捕获到
            titleInput.dispatchEvent(new Event('input', { bubbles: true }));
        }
        
        console.log('[AI] 写作区域状态检查完成');
    }
    ...
    startMonitoring() {
        ...
        // 监听题目输入
        const titleSelectors = ['#customTitle', '#article-title', '#current-title'];
    ...
    updateTopic() {
        ...
        const selectors = [
            '#customTitle',           // 🎯 主要：自定义题目输入框
            '#current-topic-title',   // 写作页面的题目显示
            '#current-title',         // 当前题目标题
            '#article-title',         // 文章标题输入框
            '#test-title',           // 测试页面专用
            '.writing-title h1',
            '.writing-title',
            '.task-title',
        ...
        const topic = this.getFirstText(selectors);

        // 增强调试信息
        console.log('[AI] 检测题目详情:', {
            found: topic,
            current: this.currentTopic,
            selectors: selectors,
            elementsFound: selectors.map(sel => {
                const el = document.querySelector(sel);
                return {
                    selector: sel,
                    exists: !!el,
    ...
    updateButtonStates() {
        ...
        // 调试模式：暂时启用所有按钮用于测试
        const debugMode = true; // 🎯 临时开启调试模式，方便测试
        
        if (debugMode) {
            // 调试模式：强制启用按钮，但如果没有题目就使用默认题目
            if (analyzeBtn) analyzeBtn.disabled = false;
            if (inspirationBtn) inspirationBtn.disabled = false;
            if (improveBtn) improveBtn.disabled = false;
            if (continueBtn) continueBtn.disabled = false;
            
            // 如果没有检测到题目，使用默认题目
            if (!this.currentTopic) {
                this.currentTopic = '我的理想';  // 设置默认题目
                const topicDisplay = document.getElementById('current-topic-display');
                if (topicDisplay) topicDisplay.textContent = this.currentTopic;
            }
        } else {
            // 正常模式
